...
def test_args_smoke(temp_project, user, request):
    ...
    model = wait_for_async_model_creation(temp_project.id,
                                          temp_project.train(trainable,
                                                             sample_pct=64))
    arguments = ('--host http://{webhost}/api'
                 ' --user {username}'
                 ' --password {password}'
                 ' {project_id}'
                 ' {model_id}'
                 ...
                 ' --n_samples 10'
                 ' --n_concurrent 1').format(webhost=webhost, username=user[0],
                                             password=user[1],
...
def test_args_from_subprocess(temp_project, user, request):
    # train one model in project
    webhost = request.config.getoption("webhost")
    trainable = temp_project.get_blueprints()[0]
    model = wait_for_async_model_creation(temp_project.id,
                                          temp_project.train(trainable,
                                                             sample_pct=64))
    arguments = ('batch_scoring --host http://{webhost}/api'
                 ' --user {username}'
                 ' --password {password}'
                 ' {project_id}'
                 ' {model_id}'
                 ' tests/fixtures/temperatura_predict.csv'
                 ' --n_samples 10'
                 ' --n_concurrent 1').format(webhost=webhost,
                                             username=user[0],
                                             password=user[1],
                                             project_id=temp_project.id,
                                             model_id=model.id)
